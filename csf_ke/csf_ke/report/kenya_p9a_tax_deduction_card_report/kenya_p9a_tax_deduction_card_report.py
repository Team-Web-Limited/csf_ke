...
def execute(filters=None):
	...
	for emp in employees:
		for month_idx in range(1, no_of_months):
			...
			basic_salary_amt = get_p9a_tax_deduction_card_amt(
				filters,
				emp.name,
				month_start_date,
				month_end_date,
			...
			benefits_non_cash_amt = get_p9a_tax_deduction_card_amt(
				filters,
				emp.name,
				month_start_date,
				month_end_date,
			...
			value_of_quarters_amt = get_p9a_tax_deduction_card_amt(
				filters,
				emp.name,
				month_start_date,
				month_end_date,
			...
			total_gross_pay_amt = get_p9a_tax_deduction_card_gross_pay(
				filters,
				emp.name,
				month_start_date,
				month_end_date,
			...
			e2_defined_contribution_retirement_scheme_amt = get_p9a_tax_deduction_card_amt(
				filters,
				emp.name,
				month_start_date,
				month_end_date,
			...
			retirement_contribution_and_owner_occupied_interest_amt = (
				lowest_of_column_e + owner_occupied_interest_amt
			)
			chargeable_pay_amt = get_p9a_tax_deduction_card_amt(
				filters,
				emp.name,
				month_start_date,
				month_end_date,
			...
			personal_relief_amt = get_p9a_tax_deduction_card_amt(
				filters,
				emp.name,
				month_start_date,
				month_end_date,
			...
			insurance_relief_amt = get_p9a_tax_deduction_card_amt(
				filters,
				emp.name,
				month_start_date,
				month_end_date,
				p9a_tax_deduction_card_type[14],
				currency,
				company_currency,
			)
			paye_tax_amt = get_p9a_tax_deduction_card_amt(
				filters,
				emp.name,
				month_start_date,
				month_end_date,
				p9a_tax_deduction_card_type[15],
				currency,
				company_currency,
			)
...
def get_employees(filters):
	...
	employee_query = (
		...
		.select(employees_doc.name, employees_doc.company)
		.where(
			(employees_doc.name == filters.get("employee"))
			& (employees_doc.company == filters.get("company"))
		)
		.orderby((employees_doc.name) & (employees_doc.company))
	)
...
def get_p9a_tax_deduction_card_amt(
	filters,
	employee,
	month_start_date,
	month_end_date,
	...
	salary_slip_query = (
		...
		.select(
			...
			salary_slip_doc.currency,
			salary_slip_doc.start_date,
			salary_slip_doc.end_date,
			salary_slip_doc.company,
			(salary_detail_doc.amount.as_("amt") if salary_detail_doc.amount else 0).as_("amt"),
			salary_slip_doc.exchange_rate,
...
def get_p9a_tax_deduction_card_gross_pay(
	filters, employee, month_start_date, month_end_date, currency, company_currency
):
	currency_filter = ""
	if filters.get("currency") and filters.get("currency") != company_currency:
		currency_filter = currency
	...
	salary_slip_query = (
		...
		.select(
			...
			salary_slip_doc.end_date,
			salary_slip_doc.company,
			(salary_slip_doc.gross_pay.as_("amt") if salary_slip_doc.gross_pay else 0).as_("amt"),
			salary_slip_doc.exchange_rate,
		)
		.where(
			(salary_slip_doc.docstatus == 1)
...
def get_p9a_tax_deduction_card_fixed_component_amt(p9a_tax_deduction_card_type):
	...
	salary_component_query = (
		frappe.qb.from_(salary_component_doc)
		.select((salary_component_doc.amount.as_("amt") if salary_component_doc.amount else 0).as_("amt"))
		.where(salary_component_doc.p9a_tax_deduction_card_type == p9a_tax_deduction_card_type)
	)
